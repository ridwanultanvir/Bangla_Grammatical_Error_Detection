    ...
    while word_pos < len(words):
        ...
        with torch.no_grad():
            if args.use_crf:
                ...
                # print('y_predict: ', y_proba)
                # for row in y_proba:
                #     print(' '.join([str(round(x.item(), 3)) for x in row]))
        for i in range(y_mask.shape[0]):
            if y_mask[i] == 1:
                ...
                decode_idx += 1
                if pred == 0: continue
                print(words_original_case[decode_idx-1], punctuation_map[pred], y_proba[i].tolist())
                orig_idx = original_text.find(words_original_case[decode_idx-1])
                # If there is a $ after the word, add it back
                # print('orig_idx: ', orig_idx)
                punct_idx = orig_idx + len(words_original_case[decode_idx-1])
