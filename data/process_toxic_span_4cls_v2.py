...
def get_spans(sentence):
  ...
  for i, span in enumerate(span_indices):
    ...
    if span[1] - span[0] < 3: # ignore $$ for now
      ...
      start -= 1
      Es.append(start)
      # print("Added to Es: ", start)
      # print(out_sentence.replace(" ", "X")[start])
      # print(out_sentence[:start] + "$" + out_sentence[start:])
      # exit()
      continue
  ...
  if count_ignore > 0:
    with open("processed_data/ignored_4cls_v2.txt", "a", encoding="utf-8") as f:
      f.write(sentence + "\n")
...
if __name__ == "__main__":
  with open("processed_data/ignored_4cls_v2.txt", "w", encoding="utf-8") as f:
    f.write("")
  ...
  print(stat)
  df.to_csv("processed_data/DataSetFold1_processed_4cls_v2.csv", index=False)
  ...
  train.to_csv("processed_data/train_4cls_v2.csv", index=False)
  test.to_csv("processed_data/test_4cls_v2.csv", index=False)
