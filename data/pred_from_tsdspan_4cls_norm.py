...
if __name__ == "__main__":
  ...
  alignments = []
  for (i, row), (j, row2) in zip(test_orig.iterrows(), test_tsd.iterrows()):
  ...
  for iiiii, (pred_3, pred_3_I, pred_3_E, test_tsd_3, test_tsd_3_orig, alignment) \
    in enumerate(zip(pred.iloc[:, 1], pred.iloc[:, 2], pred.iloc[:, 3],
          test_tsd['text'], test_orig['text'], alignments)):
