...
def combine_E(files, type="union"):
  text = {}
  if type == "union":
    fn = binary_union
  else:
  ...
  for fil in files:
    with open(fil, "r") as f:
      for line in f:
        ...
        if int(line_split[0]) in text:
          text[int(line_split[0])] = fn(
            text[int(line_split[0])], eval(line_split[3])
          )
        else:
          text[int(line_split[0])] = eval(line_split[3])
  return text
...
def write_dict_to_file(text, text_I, text_E, path):
  with open(path, "w") as f:
    for id, spans in text.items():
      # if id != len(text) - 1:
      if 1:
        f.write(f"{id}\t{str(spans)}\t{str(text_I[id])}\t{str(text_E[id])}\n")
      # else:
...
if __name__ == "__main__":
  ...
  text_E = combine_E(combine_config.files, combine_config.type)

  dir = "/".join(combine_config.path.split("/")[:-1])
  if not os.path.exists(dir):
    os.makedirs(dir)
  write_dict_to_file(text, text_I, text_E, combine_config.path)
